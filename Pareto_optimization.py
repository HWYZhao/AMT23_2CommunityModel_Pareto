...
"""
Multi-objective optimization for two-layer biogeochemical model
Optimizes parameters to fit both chlorophyll and particulate carbon profiles
"""
...
def objective_cp(params, X_carbon, Y, S1):
    ...
    CHL_MLD = MLD_pop * S1
    CHL_DCM = DCM_pop * S1
    model = Cp1 * CHL_MLD + Cp2 * CHL_DCM
    
    return np.sum((model - X_carbon)**2)

def validate_constraints(params, MLD_OD):
    """
    Validate physical constraints on model parameters
    
    Constraints ensure:
    - Mixed layer depth is above DCM depth
    - DCM depth is sufficiently below mixed layer
    - Gaussian width is physically reasonable
    """
    P2, P3, P4, P5, Cp1, Cp2 = params
    P1 = 10**(0.08 * P2 + 0.66)
...
def is_dominated(p1, p2):
    """Check if solution p1 is dominated by solution p2 in Pareto sense"""
    return (p2[0] <= p1[0] and p2[1] <= p1[1]) and (p2[0] < p1[0] or p2[1] < p1[1])

def select_best_solutions(population, pop_size):
    """
    Select non-dominated solutions for next generation
    """
...
def select_balanced_solution(population):
    """
    Select solution with best balance between both objectives
    
    Chooses solution closest to ideal point (minimum for both objectives)
...
def plot_results(population, selected_sol, f1_hist, f2_hist, param_hist, param_names, save_path=None, station_name=None):
    """
    Generate comprehensive optimization results figure
    
    Creates multi-panel figure showing:
    - Pareto front with selected solution
    - Convergence histories for both objectives
...
def pareto_optimization(bounds, args, MLD_OD, wuyi_params, pop_size=200, gens=100, plot_dir=None, station_name=None):
    ...
    if not population:
        raise ValueError("Cannot generate valid initial solutions")
...
def process_file(file_path):
    """
    Process single station file with Pareto optimization
    
    Reads initial parameters from previous optimization,
    performs multi-objective optimization, and saves results
    """
    try:
        ...
        if filename in skip_files:
            print(f"Skipping {filename}: one community only")
            return False
        ...
        try:
            ...
            print(f"Failed to retrieve initial parameters: {str(e)}")
            return False
...
def generate_summary(base_path):
    """
    Generate summary statistics comparing initial and optimized parameters
    
    Creates detailed and pivot table summaries showing parameter changes
    across all processed stations
    """
    results = []
...
def main():
    ...
    Processes all station files in specified directory structure,
    performs Pareto optimization, and generates summary statistics
    """
    ...
    for folder in os.listdir(base_path):
        ...
        if os.path.isdir(folder_path) and folder.isdigit():
            ...
            for file in files:
                print(f'\nProcessing {os.path.basename(file)} in folder {folder}')
